...
async function sleeper(path) {
  const response = await fetch(`${BASE}${path}`, {
    headers: {
      "user-agent": "barmstedt-beavers-snapshot/2.0",
    },
  });
...
function normalizePlayer(playerId, playersById) {
  ...
  // Team defenses are represented by strings such as "HOU".
...
function isFantasyRelevant(player) {
  ...
  // Exclude retired/inactive historical entries as much as possible.
...
async function main() {
  ...
  /*
   * Sleeper's NFL state is preseason right now.
   * For league data before Week 1 we still use fantasy Week 1.
   */
  const week =
  ...
  const myRoster =
    rosterViews.find(
      (roster) =>
        roster.owner_id === profile.user_id
    ) || null;
